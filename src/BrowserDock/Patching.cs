...
internal static class DriverPatchCache
{
    ...
    private static string Prepare(DriverArtifactOptions options, Version version, DriverPatchMode mode, CancellationToken token)
    {
        ...
        var root = Paths.Canonical(options.CacheDirectory ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "BrowserDock", "drivers"));
        var key = Hash(Encoding.UTF8.GetBytes($"{sourceHash}:{version}:{strategy.RecipeId}"));
        Directory.CreateDirectory(root);
        using var mutex = new Mutex(false, "BrowserDock.Patch." + key);
        var held = false;
        string? staging = null;
        try
        {
    ...
    private static BrowserDockException Mismatch(string message) => new(ErrorCategory.PatchMismatch, message);
}
