    ...
    public static string Canonical(string path)
    {
        ...
        for (var current = full; !string.IsNullOrEmpty(current); current = Path.GetDirectoryName(current))
            if ((File.Exists(current) || Directory.Exists(current)) && (File.GetAttributes(current) & FileAttributes.ReparsePoint) != 0)
                throw new BrowserDockException(ErrorCategory.ConfigurationError, "Symbolic links and reparse points are not accepted for owned resources.");
        return RuntimeCompatibility.TrimDirectorySeparator(full);
    }
}
internal sealed class Profile : IDisposable
{
    private const string Marker = ".browserdock-owner.json";
    ...
    public static Profile Acquire(ProfileOptions options)
    {
        ...
        var root = Paths.Canonical(System.IO.Path.Combine(Paths.TempRoot, "BrowserDock"));
        ...
        foreach (var vendor in new[] { "Google/Chrome/User Data", "Google/Chrome Beta/User Data", "Google/Chrome SxS/User Data", "Chromium/User Data" })
        {
            ...
            if (path.Equals(forbidden, StringComparison.OrdinalIgnoreCase) || path.StartsWith(forbidden + System.IO.Path.DirectorySeparatorChar, StringComparison.OrdinalIgnoreCase))
                throw new BrowserDockException(ErrorCategory.ConfigurationError, "The default browser profile cannot be used.");
        }
        Directory.CreateDirectory(path);
        FileStream fileLock;
        try { fileLock = new FileStream(System.IO.Path.Combine(path, ".browserdock.lock"), FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None); }
        catch (IOException e) { throw new BrowserDockException(ErrorCategory.ConfigurationError, "The profile is already in use.", e); }
    ...
    {
        if (!chromeExited) throw new BrowserDockException(ErrorCategory.CleanupIncomplete, "Chrome is still alive; the profile remains locked and is not deleted.");
        Dispose();
        if (!Temporary) return;
        var root = Paths.Canonical(System.IO.Path.Combine(Paths.TempRoot, "BrowserDock"));
        if (!Paths.Canonical(Path).StartsWith(root + System.IO.Path.DirectorySeparatorChar, StringComparison.Ordinal))
            throw new BrowserDockException(ErrorCategory.CleanupIncomplete, "Temporary profile escaped its ownership root.");
...
internal sealed class OwnedProcess : IDisposable
{
    ...
    public OwnedProcess(string path, IEnumerable<string> arguments, bool tree = false)
    {
        ...
        {
            process = Process.Start(info) ?? throw new BrowserDockException(ErrorCategory.ChromeStartFailure, "Process creation returned no process.");
    ...
    private void KillExact()
    {
        ...
        if (process.StartTime.ToUniversalTime() != Created || !string.Equals(Paths.Canonical(process.MainModule!.FileName), executable, StringComparison.OrdinalIgnoreCase))
            throw new BrowserDockException(ErrorCategory.CleanupIncomplete, "Process identity changed; refusing termination.");
        process.Kill();
    }
...
internal static class BrowserHosting
{
    ...
    public static string LocateChrome(string? specified)
    {
        ...
        return candidates.Where(File.Exists).Select(Paths.Canonical).FirstOrDefault() ?? throw new BrowserDockException(ErrorCategory.ConfigurationError, "Chrome executable was not found.");
    }
    public static async Task<Version> ValidateVersionsAsync(string chrome, string driver, CancellationToken token)
    {
        if (!File.Exists(driver)) throw new BrowserDockException(ErrorCategory.ConfigurationError, "ChromeDriver executable was not found.");
        ...
        using var process = Process.Start(info) ?? throw new BrowserDockException(ErrorCategory.VersionMismatch, "Unable to read driver version.");
    ...
    internal static Version ParseVersion(string value)
    {
        ...
        if (!match.Success) throw new BrowserDockException(ErrorCategory.VersionMismatch, "Could not parse a four-part browser/driver version.");
    ...
    internal static void RequireMatchingVersions(Version chrome, Version driver)
    {
        if (chrome.Major < 115 || chrome.Major != driver.Major || chrome.Minor != driver.Minor || chrome.Build != driver.Build)
            throw new BrowserDockException(ErrorCategory.VersionMismatch, "Chrome and ChromeDriver must match MAJOR.MINOR.BUILD (M115+).");
    }
