...
internal static class ScriptValues
{
    ...
    {
        if (depth > 64) throw new UcException(ErrorCategory.ProtocolError, "Script value exceeds the nesting limit.");
        ...
        if (value is IWebElement or IWebDriver) throw new UcException(ErrorCategory.ConfigurationError, "Script values cannot contain Selenium objects. Use locator-based ElementRef operations.");
        if (value is System.Collections.IDictionary dictionary)
        {
            var result = new Dictionary<string, object?>();
            foreach (System.Collections.DictionaryEntry entry in dictionary)
            {
                if (entry.Key is not string key) throw new UcException(ErrorCategory.ConfigurationError, "Script object keys must be strings.");
        ...
        throw new UcException(ErrorCategory.ConfigurationError, "Script values must be JSON-compatible scalars, arrays or string-keyed dictionaries.");
    }
}
public sealed class ElementRef
{
    private readonly UcBrowser browser;
    ...
    internal ElementRef(UcBrowser browser, string id, Locator locator, FindOptions options, TargetKey target, long generation, long epoch)
