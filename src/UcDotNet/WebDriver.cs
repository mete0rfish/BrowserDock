...
internal sealed class Attachment(OwnedProcess process, int port, DetachAwareCommandExecutor executor, RemoteWebDriver driver)
{
    ...
    public static async Task<Attachment> CreateAsync(string path, Uri cdp, BrowserTimeouts timeouts, CancellationToken token)
    {
        ...
        for (var attempt = 0; attempt < 3; attempt++)
        {
            ...
            catch (Exception e)
            {
                ...
                catch (Exception cleanupError) { throw new UcException(ErrorCategory.CleanupIncomplete, "Attachment creation cleanup did not finish.", e) { CleanupFailures = [cleanupError.GetType().Name] }; }
                token.ThrowIfCancellationRequested();
                if (creatingSession) throw new UcException(ErrorCategory.WebDriverSessionFailure, "Could not create a WebDriver session.", e) { Retryable = true };
            }
        }
        throw new UcException(ErrorCategory.DriverProcessFailure, "ChromeDriver failed readiness after three port attempts.", last);
    }
    internal static By By(Locator locator) => locator.Kind switch
    {
        ...
        _ => throw new UcException(ErrorCategory.ConfigurationError, "Unknown locator kind.")
    };
    internal static UcException Map(Exception e) => e switch
    {
        UcException uc => uc,
