...
public sealed class UcBrowser
{
    ...
    public static Task<UcBrowser> StartAsync(BrowserOptions options, CancellationToken cancellationToken = default)
    {
        if (options is null) throw new ArgumentNullException(nameof(options));
        var snapshot = options.Snapshot();
        return Api.Call(async () => new UcBrowser(await Core.UcBrowser.StartAsync(snapshot, cancellationToken).ConfigureAwait(false)));
    }
