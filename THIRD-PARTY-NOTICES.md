...
- SeleniumBase 4.53.7 and psutil 7.0.0 are development-only Python dependencies of the optional reference runner. They are installed separately, are not copied into the source tree, and are not included in either NuGet package. See `tests/seleniumbase-reference/requirements.txt` and its pinned scenario provenance; preserve their upstream license material if redistributing that Python environment.
