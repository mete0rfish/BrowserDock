...
public sealed class WindowsLegacyTests
{
    ...
    public void RequireWindowsFixture()
    {
        ...
        if (!File.Exists(chrome) || !File.Exists(driver)) Assert.Ignore("Set BROWSERDOCK_CHROME and BROWSERDOCK_DRIVER to a compatible fixture pair.");
        options = new() { ChromeBinaryPath = chrome, Driver = new() { ExecutablePath = driver! }, NewDocumentScripts = new[] { "globalThis.__browserDockFixture='installed'" } };
    }
        ...
        try
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    var state = await fresh.Commands.ExecuteScriptAsync("return [localStorage.getItem('fixture'),document.cookie.includes('fixture=kept'),document.body.dataset.marker,globalThis.__browserDockFixture]");
                    ...
                    // SendKeys changes the live DOM property, not the HTML value attribute.
                    var frameValue = await fresh.Commands.ExecuteScriptAsync("return document.getElementById('frame-input').value");
                    Assert.That(frameValue.GetString(), Is.EqualTo("framework"));
                }
    ...
    public void StartupCancellationRollsBackAndUnlocksCallerProfile(string stage)
    {
        ...
        try
        {
            ...
            Assert.CatchAsync<OperationCanceledException>(async () => await Browser.StartAsync(snapshot, canceled.Token));
    ...
    public async Task NavigationCancellationAndCanceledStopReleaseOwnedResources()
    {
        ...
        try
        {
            ...
            using var server = await FixtureServer.StartAsync();
            var core = await Browser.StartAsync(options.Snapshot());
            var browser = new Legacy.Browser(core);
            try
            {
                using var chrome = Process.GetProcessById(browser.Health.ChromePid!.Value);
    ...
    public async Task HundredCyclesReleaseTemporaryProfilesProcessesAndPorts()
    {
        ...
        for (var i = 0; i < 105; i++)
        {
            ...
            try
            {
                ...
                var resources = core.CaptureResourcesForTest();
                using var chrome = Process.GetProcessById(browser.Health.ChromePid!.Value);
                using var driver = Process.GetProcessById(browser.Health.DriverPid!.Value);
                await browser.StopAsync();
                Assert.That(resources(), Is.EqualTo((0, 0, 0, false)));
                Assert.That(core.StoppedTreeAliveForTest, Is.False);
    ...
    public async Task TwentyInstancesUseDistinctPorts()
    {
        ...
        try
        {
            await Task.WhenAll(Enumerable.Range(0, 20).Select(async _ => started.Add(await Browser.StartAsync(options.Snapshot()))));
        ...
        finally { await Task.WhenAll(started.Select(x => new Legacy.Browser(x).DisposeAsync())); }
    }

    private static string NewDirectory() { var path = Path.Combine(Path.GetTempPath(), "BrowserDock-framework-" + Guid.NewGuid().ToString("N")); Directory.CreateDirectory(path); return path; }
    private static void RequireStress() { if (Environment.GetEnvironmentVariable("BROWSERDOCK_STRESS") != "1") Assert.Ignore("Set BROWSERDOCK_STRESS=1 for resource-intensive tests."); }
