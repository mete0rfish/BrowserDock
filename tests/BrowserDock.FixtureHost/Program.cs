...
var driverReport = Environment.GetEnvironmentVariable("BROWSERDOCK_FAKE_DRIVER_REPORT");
var portArgument = args.SingleOrDefault(x => x.StartsWith("--port=", StringComparison.Ordinal));
var driverPort = driverReport is not null && portArgument is not null ? int.Parse(portArgument.Substring(7)) : 0;
builder.WebHost.ConfigureKestrel(server => server.Listen(IPAddress.Loopback, driverPort));
var app = builder.Build();
if (driverPort != 0)
{
    // A real child process with a real listener, but a deliberately stalled
    // W3C new-session response. The product must kill and drain this process.
    app.MapGet("/status", () => Results.Json(new { value = new { ready = true } }));
    app.MapPost("/session", async (HttpContext context) =>
    {
        await File.WriteAllTextAsync(driverReport! + ".tmp", JsonSerializer.Serialize(new { pid = Environment.ProcessId, port = driverPort }));
        File.Move(driverReport + ".tmp", driverReport!);
