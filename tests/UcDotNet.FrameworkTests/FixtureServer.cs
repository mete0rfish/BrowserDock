...
internal sealed class FixtureServer : IDisposable
{
    ...
    {
        var configured = Environment.GetEnvironmentVariable("UCDOTNET_FIXTURE_HOST");
        if (string.IsNullOrEmpty(configured))
        {
            ...
            if (directory is null) throw new InvalidOperationException("Set UCDOTNET_FIXTURE_HOST to the fixture host DLL.");
            var configuration = new DirectoryInfo(TestContext.CurrentContext.TestDirectory).Parent!.Name;
            configured = Path.Combine(directory.FullName, "tests", "UcDotNet.FixtureHost", "bin", configuration, "net10.0", "UcDotNet.FixtureHost.dll");
        }
        ...
        try
        {
            ...
            while (true)
            {
                ...
                if (line.StartsWith("UCDOTNET_FIXTURE_URL=", StringComparison.Ordinal)) return new(process, new Uri(line.Substring("UCDOTNET_FIXTURE_URL=".Length)), stderr);
            }
        }
