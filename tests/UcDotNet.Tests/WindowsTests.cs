...
public sealed class WindowsTests
{
    ...
    public void RequireWindowsFixture()
    {
        ...
        if (string.IsNullOrEmpty(chrome) || string.IsNullOrEmpty(driver) || !File.Exists(chrome) || !File.Exists(driver)) Assert.Ignore("Set UCDOTNET_CHROME and UCDOTNET_DRIVER to a compatible fixture pair.");
        options = new() { ChromeBinaryPath = chrome, Driver = new() { ExecutablePath = driver! }, NewDocumentScripts = ["globalThis.__ucFixture = 'installed'"] };
    }
    internal static Task<LocalServer> FixtureAsync() => LocalServer.StartAsync(async context =>
    {
        ...
        await context.Response.WriteAsync("""
            <!doctype html><title>UcDotNet fixture</title><button id="button" onclick="this.textContent='clicked'">ready</button>
    ...
    public async Task AC01_02_03_DisconnectPreservesChromeAndReconnectReplacesSession()
    {
        ...
        await using var browser = await UcBrowser.StartAsync(options);
        ...
        Assert.That(Assert.ThrowsAsync<UcException>(async () => await reacquired.GetTextAsync())!.Category, Is.EqualTo(ErrorCategory.StaleDomElement));
    }
    ...
    public async Task ExplicitTargetsAndSocketRecoveryRestoreScripts()
    {
        ...
        var result = await browser.ExecuteCdpAsync("Runtime.evaluate", new { expression = "globalThis.__ucFixture", returnByValue = true });
    ...
    public async Task AC05_TwentyInstancesHaveDistinctEndpointsAndPorts()
    {
        ...
        try
        {
            await Task.WhenAll(Enumerable.Range(0, 20).Select(async _ => started.Add(await UcBrowser.StartAsync(options))));
    ...
    public async Task AC04_09_HundredCyclesDoNotLeakOwnedResources()
    {
        ...
        TestContext.Out.WriteLine(JsonSerializer.Serialize(new { baseline, final, medianBytes }, new JsonSerializerOptions { IncludeFields = true }));
    ...
    public async Task AC06_StartupCancellationRollsBack(string stage)
    {
        ...
        try
        {
            ...
            Assert.ThrowsAsync<OperationCanceledException>(async () => await UcBrowser.StartAsync(configured, cancellation.Token));
    ...
    {
        var browser = await UcBrowser.StartAsync(options);
        try
        {
            ...
            Assert.ThrowsAsync<UcException>(async () => await lease.Commands.GetUrlAsync());
            Assert.That(browser.State, Is.EqualTo(BrowserState.CdpOnly));
            await browser.ReconnectWebDriverAsync();
    ...
    public async Task AC07_AttachedResizeReportsCapabilityResult()
    {
        ...
        try { await lease.Commands.ResizeWindowAsync(1024, 768); TestContext.Out.WriteLine($"{version}: attached resize supported"); }
        catch (UcException e) when (e.Category == ErrorCategory.UnsupportedAttachedCommand) { TestContext.Out.WriteLine($"{version}: attached resize unsupported"); }
    }
    [TestCase(false, false), TestCase(false, true), TestCase(true, false), TestCase(true, true)]
    public async Task AC01_DetachCapabilityAndDeleteSessionComparison(bool detach, bool sendDelete)
    {
        await using var unrelated = await UcBrowser.StartAsync(options);
        await using var browser = await UcBrowser.StartAsync(options);
        await browser.DisconnectWebDriverAsync();
        ...
        try
        {
            ...
            var cdpAlive = false;
            if (survived) { try { await browser.ExecuteCdpAsync("Browser.getVersion"); cdpAlive = true; } catch (UcException) { } }
    ...
    public async Task AC06_NavigationAndReconnectCancellationPreserveRecoverableBrowser()
    {
        ...
        await using var browser = await UcBrowser.StartAsync(configured);
        await browser.DisconnectWebDriverAsync();
    ...
    public async Task CallerProfileAndCanceledStopPreserveSentinelAndReleaseLock()
    {
        ...
        try
        {
            ...
            var browser = await UcBrowser.StartAsync(options with { Profile = new() { Directory = root } });
    ...
    private static void RequireStress() { if (Environment.GetEnvironmentVariable("UCDOTNET_STRESS") != "1") Assert.Ignore("Set UCDOTNET_STRESS=1 to run resource-intensive acceptance tests."); }
