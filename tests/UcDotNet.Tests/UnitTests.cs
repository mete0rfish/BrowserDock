...
public sealed class UnitTests
{
    ...
    public void VersionsRequireExactBuild(string chrome, string driver, bool accepted)
    {
        ...
        else Assert.That(Assert.Throws<UcException>(() => BrowserHosting.RequireMatchingVersions(Version.Parse(chrome), Version.Parse(driver)))!.Category, Is.EqualTo(ErrorCategory.VersionMismatch));
    }
    ...
    public void NavigationMatrixRejectsContradictions(BrowserState state, NavigationMode mode, bool reconnect, bool accepted)
    {
        ...
        if (accepted) Assert.DoesNotThrow(() => UcBrowser.ValidateNavigation(state, options));
        else Assert.Throws<UcException>(() => UcBrowser.ValidateNavigation(state, options));
    }
    ...
    public void PatchPreservesSourceAndFailsOnUnexpectedCounts()
    {
        ...
        Assert.Throws<UcException>(() => DriverPatchCache.Apply(bytes, new Recipe(2), out _));
        Assert.Throws<UcException>(() => DriverPatchCache.Apply(Encoding.ASCII.GetBytes("unknown"), new Recipe(), out _));
    }
    [Test]
    public async Task PatchCacheIsAtomicIdempotentAndDetectsTampering()
    {
        ...
        try
        {
            ...
            Assert.That(Assert.ThrowsAsync<UcException>(async () => await DriverPatchCache.PrepareAsync(options, new Version(150, 0, 1, 0), DriverPatchMode.BinaryCompatibility, default))!.Category, Is.EqualTo(ErrorCategory.PatchMismatch));
        }
    ...
    public async Task UnknownRecipeFailsClosedWhileDisabledNeedsNoRecipe()
    {
        ...
        Assert.ThrowsAsync<UcException>(async () => await DriverPatchCache.PrepareAsync(options, new Version(150, 0), DriverPatchMode.BinaryCompatibility, default));
    }
    [Test]
    public async Task CallerProfileIsExclusivelyLockedAndPreservesSentinel()
    {
        ...
        try
        {
            ...
            Assert.Throws<UcException>(() => Profile.Acquire(new() { Directory = root }));
    ...
    public void RejectsReparsePointOwnedPaths()
    {
        ...
        var root = TestPaths.NewDirectory();
        try { Directory.CreateSymbolicLink(Path.Combine(root, "link"), root); Assert.Throws<UcException>(() => Paths.Canonical(Path.Combine(root, "link", "profile"))); }
        finally { Directory.Delete(root, true); }
    }
    [Test]
    public async Task TemporaryProfileRequiresMatchingMarkerBeforeDeletion()
    {
        ...
        var marker = Path.Combine(profile.Path, ".ucdotnet-owner.json");
        var original = await File.ReadAllTextAsync(marker);
        await File.WriteAllTextAsync(marker, "{\"Schema\":99,\"Nonce\":\"wrong\",\"Path\":\"wrong\"}");
        Assert.ThrowsAsync<UcException>(async () => await profile.CleanupAsync(true, default));
    ...
    public void LiveBrowserPreventsProfileDeletionAndUnlock()
    {
        ...
        try
        {
            ...
            Assert.ThrowsAsync<UcException>(async () => await profile.CleanupAsync(false, default));
            Assert.Throws<UcException>(() => Profile.Acquire(new() { Directory = root }));
        }
        finally { Directory.Delete(root, true); }
    }
    ...
    public void SourceDoesNotDependOnSeleniumPrivateApisOrNameBasedKill()
    {
        ...
        while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "UcDotNet.slnx"))) directory = directory.Parent;
        Assert.That(directory, Is.Not.Null);
        var files = Directory.GetFiles(Path.Combine(directory!.FullName, "src", "UcDotNet"), "*.cs", SearchOption.TopDirectoryOnly);
...
internal static class TestPaths
{
    public static string NewDirectory()
    {
        ...
        var path = Path.Combine(root, "ucdotnet-test-" + Guid.NewGuid().ToString("N"));
